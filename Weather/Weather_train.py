...
# 数据加载
file_path = r"D:\Code\cx\data\weather.csv"
...
# 特征和目标列
features = ['p', 'T', 'Tdew', 'rh', 'VPmax', 'VPact', 'VPdef',
            'sh', 'H2OC', 'rho', 'wv', 'max_wv', 'wd', 'rain', 'raining',
            'SWDR', 'PAR', 'max_PAR', 'Tlog', 'month', 'day', 'hour', 'minute']
target = 'Tpot'
...
def create_sequences(data, time_steps):
    ...
    for i in range(len(data) - time_steps):
        # 主输入是目标变量 OT 的历史时间步
        X_main.append(data['Tpot'].iloc[i:i + time_steps].values)

        # 辅助输入是其他特征的历史时间步
        X_aux.append(data[features].iloc[i:i + time_steps].values)

        # 输出是目标变量 OT 的下一步
        y.append(data['Tpot'].iloc[i + time_steps])
...
r = 24
learning_rate = 2.3989425914944447e-05
dropout_rate = 0.1
# 保存数据到CSV文件
name = "HOES"
save_path = r"D:\Code\cx\Weather"
train_save_path = r"D:\Code\cx\Weather\train"
